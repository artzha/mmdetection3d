...
class CODaInfoGatherer:
    """
    Parallel version of waymo dataset information gathering.
    WayCODamo annotation format version like KITTI:
    {
        [optional]points: [N, 3+] point cloud
        [optional, for kitti]image: {
            image_idx: ...
    ...
    def gather_single(self, idx):
        ...
        if self.calib:
            ...
            Tr_velo_to_cam1 = np.array([
                float(info) for info in lines[4].split(' ')[1:13]
            ]).reshape([3, 4])
            if self.extend_matrix:
            ...
            calib_info['P1'] = P1
            calib_info['R0_rect'] = rect_4x4
            calib_info['Tr_velo_to_cam'] = Tr_velo_to_cam0
